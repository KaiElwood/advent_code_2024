...
fn pt2() {
    ...
    // let final = count_xmas_2(arrays, map);
    // so I will run the regex but return the position of a of any strings that match, then get that usize from the tuple and add that to the map
}
...
fn count_xmas_2(array: &Vec<(usize, char)>, map: &mut HashMap<usize, usize>) -> usize {
    // so i want to go through the array and find pos of any matches of MAS
    //  if there is one, then take that pos and add it to map. if map already exsts, then add one to total
    let mut total = 0;
